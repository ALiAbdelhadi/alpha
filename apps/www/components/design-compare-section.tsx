...
export function DesignCompareSection() {
    const t = useTranslations("designCompare")
    ...
    return (
        <div style={{ width: "100%", height: "100%", overflow: "hidden", background: "#f5f5f5", fontFamily: "Arial, Helvetica, sans-serif" }}>
            <div style={{ background: "#fff", padding: "0 20px", height: 44, display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid #ddd" }}>
                <span style={{ fontSize: 18, fontWeight: 900, color: "#222" }}>{t("mockupBrandName")}</span>
                <div style={{ display: "flex", gap: 16, fontSize: 11, color: "#555" }}>
                    <span>{t("mockupNavServices")}</span>
                    <span>{t("mockupNavPortfolio")}</span>
                    <span>{t("mockupNavAbout")}</span>
                    <span>{t("mockupNavBlog")}</span>
                </div>
                <button style={{ background: "#0066cc", color: "#fff", fontSize: 11, fontWeight: 700, padding: "6px 14px", borderRadius: 4, border: "none" }}>
                    {t("mockupNavCta")}
                </button>
            </div>
            <div style={{ background: "#fff", padding: "22px 20px 16px", display: "grid", gridTemplateColumns: "1fr 140px", gap: 16 }}>
                <div>
                    <div style={{ fontSize: "clamp(18px,4vw,24px)", fontWeight: 900, color: "#111", lineHeight: 1.15, textTransform: "uppercase" as const, letterSpacing: "-.01em", marginBottom: 8 }}>
                        {t("mockupHeroLine1")}<br />{t("mockupHeroLine2")}
                    </div>
                    <div style={{ fontSize: 11, color: "#555", lineHeight: 1.5, marginBottom: 14 }}>
                        {t("mockupHeroSub")}
                    </div>
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap" as const }}>
                        <button style={{ background: "#0066cc", color: "#fff", fontSize: 10, fontWeight: 700, padding: "7px 14px", borderRadius: 3, border: "none", letterSpacing: ".03em", textTransform: "uppercase" as const }}>
                            {t("mockupCta1")}
                        </button>
                        <button style={{ background: "#fff", color: "#ff6600", fontSize: 10, fontWeight: 700, padding: "6px 13px", borderRadius: 3, border: "2px solid #ff6600", letterSpacing: ".03em", textTransform: "uppercase" as const }}>
                            {t("mockupCta2")}
                        </button>
                        <button style={{ background: "#ff6600", color: "#fff", fontSize: 10, fontWeight: 700, padding: "6px 13px", borderRadius: 3, border: "2px solid #ff6600", letterSpacing: ".03em", textTransform: "uppercase" as const }}>
                            {t("mockupCta3")}
                        </button>
                    </div>
                </div>
                <div style={{ background: "#ddd", borderRadius: 4, border: "1px solid #ccc", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 9, color: "#999", textTransform: "uppercase" as const, letterSpacing: ".1em", height: 110, textAlign: "center", whiteSpace: "pre-line" }}>
                    {t("mockupImagePlaceholder")}
                </div>
            </div>
            <div style={{ background: "#fff3f0", padding: "7px 20px", borderTop: "1px solid #ffd0b8", borderBottom: "1px solid #ffd0b8", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" as const }}>
                <span style={{ background: "#ff6600", color: "#fff", fontSize: 8, fontWeight: 700, padding: "3px 7px", borderRadius: 2, textTransform: "uppercase" as const, letterSpacing: ".05em" }}>
                    {t("mockupBadgeNew")}
                </span>
                <span style={{ fontSize: 9, color: "#cc3300", fontWeight: 700 }}>🔥 {t("mockupPromoHeadline")}</span>
                <span style={{ fontSize: 9, color: "#555" }}>{t("mockupPromoSub")}</span>
                <span style={{ background: "#0066cc", color: "#fff", fontSize: 8, fontWeight: 700, padding: "3px 7px", borderRadius: 2, textTransform: "uppercase" as const, letterSpacing: ".05em" }}>
                    {t("mockupPromoCta")}
                </span>
            </div>
            <div style={{ padding: "14px 20px", display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 10 }}>
                {([
                    { titleKey: "mockupSvc1Title", descKey: "mockupSvc1Desc", c: "#0066cc" },
                    { titleKey: "mockupSvc2Title", descKey: "mockupSvc2Desc", c: "#ff6600" },
                    { titleKey: "mockupSvc3Title", descKey: "mockupSvc3Desc", c: "#0066cc" },
                ] as const).map(card => (
                    <div key={card.titleKey} style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: 4, padding: 12, boxShadow: "0 2px 6px rgba(0,0,0,.08)", borderTop: `3px solid ${card.c}` }}>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#111", marginBottom: 4, textTransform: "uppercase" as const, letterSpacing: ".03em" }}>
                            {t(card.titleKey)}
                        </div>
                        <div style={{ fontSize: 9.5, color: "#777", lineHeight: 1.5 }}>
                            {t(card.descKey)}
                        </div>
                    </div>
                ))}
            ...
            <div style={{ background: "#f0f7ff", padding: "8px 20px", borderTop: "1px solid #d0e4ff", fontSize: 9, display: "flex", gap: 16, alignItems: "center" }}>
                <span style={{ color: "#0066cc", fontWeight: 700 }}>✓ {t("mockupTrust1")}</span>
                <span style={{ color: "#555" }}>✓ {t("mockupTrust2")}</span>
                <span style={{ color: "#555" }}>✓ {t("mockupTrust3")}</span>
                <span style={{ color: "#ff6600", fontWeight: 700 }}>★★★★★ {t("mockupTrust4")}</span>
            </div>
        </div>
    )
}
        ...
        <>
            ...
            <div className="ax-root">
                ...
                <nav style={{ height: 44, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 20px", borderBottom: ".5px solid rgba(255,255,255,.07)", background: "rgba(8,8,7,.7)", backdropFilter: "blur(12px)", flexShrink: 0, position: "relative", zIndex: 2 }}>
                    <span style={{ fontSize: 13, fontWeight: 600, color: "#fff", letterSpacing: "-.025em" }}>{t("mockupBrandName")}</span>
                    <div style={{ display: "flex", gap: 16, fontSize: 10, color: "rgba(255,255,255,.32)", letterSpacing: ".06em", textTransform: "uppercase" as const }}>
                        <span>{t("axNavWork")}</span>
                        <span>{t("axNavServices")}</span>
                        <span>{t("axNavStudio")}</span>
                    </div>
                ...
                <div style={{ padding: "16px 20px 10px", flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: "clamp(20px,4vw,28px)", fontWeight: 300, color: "rgba(255,255,255,.92)", lineHeight: 1.06, letterSpacing: "-.03em" }}>
                        {t("axHeroLine1")}
                    </div>
                    <div style={{ fontSize: "clamp(20px,4vw,28px)", fontWeight: 300, fontStyle: "italic", fontFamily: "Georgia, serif", color: "rgba(255,255,255,.28)", letterSpacing: "-.02em", lineHeight: 1.06 }}>
                        {t("axHeroLine2")}
                    </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <span style={{ fontSize: 9.5, color: "rgba(255,255,255,.3)", letterSpacing: ".04em" }}>
                            {t("axSecondaryCta")} →
                        </span>
                    </div>
                </div>
                <div style={{ padding: "0 20px 12px", display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gridTemplateRows: "auto auto", gap: 6, flexShrink: 0 }}>
                    <div className="ax-metric-top" style={{ gridColumn: "1/3", background: "rgba(255,255,255,.04)", border: ".5px solid rgba(255,255,255,.1)", borderRadius: 6, padding: "11px 13px", display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 8, position: "relative", overflow: "hidden" }}>
                        <div>
                            <div style={{ fontSize: 8.5, textTransform: "uppercase" as const, letterSpacing: ".16em", color: "rgba(255,255,255,.25)", marginBottom: 5, fontVariantNumeric: "tabular-nums" as const }}>
                                {t("axMetricLabel")}
                            </div>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
                            <div style={{ fontSize: 9, fontWeight: 600, color: "#22c55e", letterSpacing: ".04em", background: "rgba(34,197,94,.1)", padding: "2px 6px", borderRadius: 10, border: ".5px solid rgba(34,197,94,.25)" }}>
                                ↑ {t("axMetricBadge")}
                            </div>
                    ...
                    {([
                        { codeKey: "SVC_03", titleKey: "axSvc3Title", descKey: "axSvc3Desc" },
                        { codeKey: "SVC_01", titleKey: "axSvc1Title", descKey: "axSvc1Desc" },
                        { codeKey: "SVC_02", titleKey: "axSvc2Title", descKey: "axSvc2Desc" },
                    ] as const).map(svc => (
                        <div key={svc.codeKey} style={{ background: "rgba(255,255,255,.03)", border: ".5px solid rgba(255,255,255,.07)", borderRadius: 6, padding: 11 }}>
                            <div style={{ fontSize: 8, letterSpacing: ".14em", color: "rgba(255,255,255,.2)", marginBottom: 6, fontVariantNumeric: "tabular-nums" as const, textTransform: "uppercase" as const }}>
                                {svc.codeKey}
                            </div>
                            <div style={{ fontSize: 10.5, fontWeight: 500, color: "rgba(255,255,255,.8)", letterSpacing: "-.01em", marginBottom: 3 }}>
                                {t(svc.titleKey)}
                            </div>
                            <div style={{ fontSize: 8.5, color: "rgba(255,255,255,.28)", lineHeight: 1.5 }}>
                                {t(svc.descKey)}
                            </div>
                        </div>
                    ))}
                    <div style={{ gridColumn: "1/4", background: "rgba(255,255,255,.02)", border: ".5px solid rgba(255,255,255,.06)", borderRadius: 6, padding: "9px 13px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        ...
                        <div style={{ display: "flex", gap: 4, flexWrap: "wrap" as const }}>
                            {["Next.js", "TypeScript", "Postgres", "Figma", "Vercel"].map(tag => (
                                <span key={tag} style={{ fontSize: 7.5, letterSpacing: ".1em", textTransform: "uppercase" as const, color: "rgba(255,255,255,.3)", border: ".5px solid rgba(255,255,255,.1)", padding: "2px 6px", borderRadius: 10 }}>
                                    {tag}
                                </span>
                            ))}
                        </div>
                ...
                <div style={{ marginTop: "auto", padding: "7px 20px", borderTop: ".5px solid rgba(255,255,255,.06)", display: "flex", alignItems: "center", gap: 10, background: "rgba(8,8,7,.5)" }}>
                    <span style={{ fontSize: 7.5, textTransform: "uppercase" as const, letterSpacing: ".2em", color: "rgba(255,255,255,.18)", whiteSpace: "nowrap", fontVariantNumeric: "tabular-nums" as const }}>
                        {t("axFooter")}
                    </span>
