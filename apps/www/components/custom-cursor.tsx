...
function CustomCursor() {
  ...
  useEffect(() => {
    ...
    const updateCursor = () => {
      ...
      const t = `translate3d(${state.currentX}px, ${state.currentY}px, 0) translate(-50%, -50%)`
      outer.style.transform = `${t} scale(${scale})`
      inner.style.transform = `${t} scale(${innerScale})`
  ...
  return (
    <>
      <div
        ref={outerRef}
        className="pointer-events-none fixed left-0 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full border border-foreground/20 bg-background/20 backdrop-blur-md transition-transform duration-300 ease-out"
        style={{
      ...
      <div
        ref={innerRef}
        className="pointer-events-none fixed left-0 top-0 rounded-full bg-brand transition-transform duration-300 ease-out"
        style={{
