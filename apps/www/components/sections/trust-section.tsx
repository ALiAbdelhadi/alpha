...
export const TrustSection = memo(function TrustSection() {
  ...
  // Use raw to get the points array from the translation file
  const points = t.raw("points") as Array<{ title: string; body: string }>;

  return (
    <section className="pt-[var(--section-y-top)] pb-[var(--section-y-bottom)] border-t border-border">
      <Container>
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between gap-8 md:gap-12 mb-16">
          <div className="space-y-3">
            <p ref={eyebrowRef} className="font-mono text-sm leading-normal tracking-wider text-xs leading-normal tracking-[0.22em] uppercase rtl:font-sans rtl:normal-case rtl:tracking-normal text-muted-foreground m-0">
              {t("eyebrow")}
            </p>
        ...
        >
          {points.map((point, i) => (
            ...
            >
              <p className="font-mono text-sm leading-normal tracking-wider text-xs leading-normal tracking-[0.22em] uppercase rtl:font-sans rtl:normal-case rtl:tracking-normal text-muted-foreground mb-6">
                {String(i + 1).padStart(2, "0")}
              </p>
              <h3 className="text-[clamp(1.5rem,2.4vw,2rem)] leading-[1.15] tracking-[-0.018em] font-medium text-foreground mb-3 tracking-tight">
                {point.title}
              </h3>
              <p className="text-[clamp(1.0625rem,1.05vw,1.125rem)] leading-[1.75] text-muted-foreground leading-relaxed">
                {point.body}
              </p>
            </div>
          ))}
        ...
        <div className="mt-10 grid gap-10 md:grid-cols-[minmax(0,1fr)_320px] md:items-start border-t border-border pt-10">
          <div>
            <p className="font-mono text-sm leading-normal tracking-wider text-xs leading-normal tracking-[0.22em] uppercase rtl:font-sans rtl:normal-case rtl:tracking-normal text-muted-foreground mb-4">
              {t("founder.eyebrow")}
            </p>
          ...
          <div className="flex flex-col justify-start">
            <ExternalDirectionalLink
              href={FOUNDER_LINK}
              className="font-mono text-sm leading-normal tracking-wider text-xs leading-normal tracking-[0.22em] uppercase rtl:font-sans rtl:normal-case rtl:tracking-normal inline-flex text-foreground transition-colors hover:text-muted-foreground"
            >
              {t("founder.linkLabel")}
            </ExternalDirectionalLink>
          </div>
        ...
        <div className="flex items-center gap-4 mt-9">
          <span className="font-mono text-sm leading-normal tracking-wider text-xs leading-normal tracking-[0.22em] uppercase rtl:font-sans rtl:normal-case rtl:tracking-normal text-muted-foreground">
