  ...
  return (
    <svg
      ...
      style={{
        ...
        width: "100%", height: "3px", overflow: "visible", pointerEvents: "none",
      }}
      viewBox="0 0 100 3"
  ...
  return (
    <div
      ...
      style={{
        ...
        borderLeft: `2px solid ${INK[note.color]}`,
    ...
    >
      <p
        ...
        style={{
          fontSize: "clamp(7px, 0.72vw, 10px)", textTransform: "uppercase",
          letterSpacing: ".18em", color: INK[note.color],
        }}
      >
        {note.label}
      </p>
...
function StageNav({
  ...
  return (
    <div style={{ flexShrink: 0, marginBottom: "clamp(4px, 0.8vh, 12px)" }}>
      <div
      ...
      {mounted && activeStage >= 0 && (
        <div className="flex items-center justify-between mt-2 md:hidden" aria-live="polite" aria-atomic="true">
          <p
            className="cb-mobile-step-label font-mono text-foreground/40"
      ...
      {mounted && !skipped && hintVisible && (
        <div className="flex justify-end mt-1.5">
          <button
            onClick={onSkip}
            className="font-mono text-foreground/30 hover:text-foreground/55 transition-colors"
            style={{
...
function AnnotationsPanel({ notes, activeTargets, label }: {
  notes: Note[]
  ...
  return (
    ...
    >
      ...
      >
        {notes.map(note => (
          <BriefNote key={note.id} note={note} shown={activeTargets.has(note.id)} />
        ))}
      </div>
    </div>
  )
...
function MobileNoteCard({ note, visible, isRtl }: {
  note: Note
  visible: boolean
  isRtl: boolean
}) {
    ...
    >
      <div
        style={{
          ...
          borderRight:  isRtl ? `3px solid ${INK[note.color]}` : "0.5px solid color-mix(in srgb, hsl(var(--foreground)) 12%, transparent)",
          borderBottom: "0.5px solid color-mix(in srgb, hsl(var(--foreground)) 12%, transparent)",
          borderLeft:   isRtl ? "none" : `3px solid ${INK[note.color]}`,
          background: "hsl(var(--background))",
        }}
      >
        <p
          className="font-mono text-sm leading-normal tracking-wider mb-1.5"
          style={{ fontSize: "7px", textTransform: "uppercase", letterSpacing: ".18em", color: INK[note.color] }}
        >
          {note.label}
        </p>
        <p style={{
          fontSize: "clamp(12px, 3.2vw, 13px)",
          color: "color-mix(in srgb, hsl(var(--foreground)) 68%, transparent)",
          lineHeight: 1.6,
        }}>
          {note.body}
        </p>
  ...
  return (
    <div
      ...
      style={{
        ...
        padding: "14px 18px",
        borderRadius: "3px",
        border: "0.5px solid color-mix(in srgb, hsl(var(--foreground)) 14%, transparent)",
        display: "flex",
        alignItems: "center",
    ...
    >
      ...
      <a
        href="#contact"
        className="cb-cta-link font-mono"
        style={{
...
export function ConsultingBriefSection() {
  ...
  const isMedium = useSyncExternalStore(
    (callback) => {
      if (typeof window === "undefined") return () => undefined
      window.addEventListener("resize", callback)
      return () => window.removeEventListener("resize", callback)
  ...
  const sectionHeight = isNarrow ? "300vh" : isMedium ? "340vh" : "380vh"
  ...
  useEffect(() => {
    ...
    mm.add("(prefers-reduced-motion: no-preference)", () => {
      chevronAnim.current = gsap.to(el, {
        y: -3, repeat: -1, yoyo: true, duration: 0.9, ease: "sine.inOut",
      })
    })
    return () => mm.revert()
  }, [])
  ...
  const goStage = useCallback((index: number) => {
    ...
    if (index > prev) {
      if (index - prev > 1) {
        const skippedTargets = collectTargets(index - 1)
        setActiveTargets(current => {
          const next = new Set(current)
          skippedTargets.forEach(t => next.add(t))
          return next
        })
      }
    ...
    <>
      <style>{`
        ...
        @media (max-width: 479px) {
          .cb-body-text { font-size: 11px; line-height: 1.65; }
        }
        @media (max-width: 767px) {
          .cb-doc-grid { border-left: none; border-right: none; }
          .cb-mobile-step-label {
        ...
        .cb-mobile-note {
          ...
          transition:
            opacity   0.38s ${CB_EASE},
            transform 0.38s ${CB_EASE};
          pointer-events: none;
        }
      ...
      <div ref={wrapperRef} style={{ height: sectionHeight }} id="brief" className="p-0 m-0 w-full">
        <Container>
            ...
            <div ref={headerRef} style={{ flexShrink: 0, marginBottom: "clamp(6px, 1.2vh, 20px)" }}>
              ...
              <div className="flex items-end justify-between gap-4 md:gap-6 flex-wrap">
                ...
                <div className="hidden lg:flex flex-col items-end gap-2">
                  <p className="font-mono text-sm leading-normal tracking-wider text-foreground/45 max-w-[38ch] text-right">
                    {t("brief.subtitle")}
                  </p>
                  <p
                    className="font-mono text-foreground/60 max-w-[38ch] text-right"
                    style={{
          ...
          >
            <Link href="/schedule" aria-label={t("cta.button")}>
              <MagneticButton
